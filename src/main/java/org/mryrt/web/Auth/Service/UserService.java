package org.mryrt.web.Auth.Service;

import lombok.extern.slf4j.Slf4j;
import org.mryrt.web.Auth.Exception.InvalidCredentials;
import org.mryrt.web.Auth.Model.*;
...
public class UserService implements UserDetailsService {
    ...
    private User _getUserId(long id) {
        return userRepository
                .findById(id)
                .orElseThrow(() -> new InvalidCredentials("id", "id %d does not exist".formatted(id)));
    }

    private User _getUserUsername(String username) {
        return userRepository
                .findByUsername(username)
                .orElseThrow(() -> new InvalidCredentials("username", "username %s does not exist".formatted(username)));
    }
    ...
    public String userLogIn(LogInRequest logInRequest) {
        ...
        if (!passwordEncoder.matches(logInRequest.getPassword(), user.getPassword()))
            throw new InvalidCredentials("password", "Wrong password");
        return jwtService.generateToken(user.getUsername());
    }

    public UserDTO userGetMe() {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        if (authentication != null && authentication.isAuthenticated())
            return new UserDTO(_getUserUsername(authentication.getName()));
        throw new InvalidCredentials("username", "username %s does not exist".formatted(authentication.getName()));
    }
